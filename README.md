...
Along the way the project solved real deployment problems (serverless-incompatible SQLite, environment variables not reaching Amplify's SSR runtime, an oversized image breaking Next.js image optimisation) and introduced a consistent premium design system across the site. A subsequent production-readiness pass added SEO metadata and structured data, security headers, rate limiting and honeypots on public forms, accessibility fixes, custom error/loading boundaries, and Prettier/ESLint tooling.
...
| Tooling | Prettier + eslint-config-prettier, ESLint (`eslint-config-next`), TypeScript strict mode |
...
- **One validation source of truth:** Zod schemas are shared between the client wizard and the API routes, so rules (age category, deadlines, phone format, image URLs) cannot drift.
...
- **Defense against malformed input at the edges:** rate limiting and honeypots on public forms, and render-side guards (e.g. image URL validation) so bad data can't take a page down.
...
- Security headers (CSP, HSTS, `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`) set centrally in `next.config.ts`.
- Per-page metadata, Open Graph tags, `sitemap.ts` and `robots.ts` (disallowing `/admin` and `/api`), and JSON-LD `SportsEvent` structured data on tournament pages for Google event rich results.
- Custom `not-found.tsx`, `error.tsx`, `global-error.tsx` and a root `loading.tsx` skeleton.

### 3.2 Design System & Public Site

- **Premium design system:** charcoal / ivory / gold Tailwind tokens (including a `--gold-ink` variant tuned for WCAG AA contrast on light surfaces), Playfair Display headings with Inter body text, reusable primitives (`Container`, `SectionHeading`, `EmptyState`, `Button`, `Card`, `Badge`).
...
- **SEO:** per-tournament `generateMetadata`, a **dynamic Open Graph image** via `next/og`, and JSON-LD `SportsEvent` structured data.
- **Validation:** registration deadline must be before the start date, the end date cannot precede the start date, and poster/brochure URLs must be well-formed (shared Zod schema in the admin forms); late registrations are rejected server-side.
...
- **Anti-spam:** rate limiting and a honeypot field, checked server-side before validation.
...
- **Analytics:** payments collected (fee-bearing tournaments only), 30-day registrations trend, category breakdown chart, registrations per tournament — charts are code-split into client-only bundles so `recharts` never ships in the initial page load.
...
- **Input validation:** poster image URLs and class banner image URLs are validated server-side (with an inline error message) before saving, so a malformed value can't reach the public site.
...
- Both the waitlist and enrolment forms are rate-limited and honeypot-protected.

### 3.11 Contact

- Validated contact form stored in `ContactMessage`, rate-limited and honeypot-protected.
...
| System | `/sitemap.xml`, `/robots.txt`, `not-found`/`error`/`global-error` boundaries |
...
| 23 Sep 2026 | Production-readiness pass: SEO metadata/JSON-LD/sitemap, security headers, rate limiting and honeypots, accessibility fixes, custom error/loading boundaries, Prettier/ESLint tooling, dependency security patches |
...
- **Rate limiting:** in-memory and per-instance (best-effort), not distributed — sufficient for current traffic but would need Redis/Upstash to scale across multiple server instances.
