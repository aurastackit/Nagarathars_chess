...
const NODE_COLORS = ["#16324f", "#157a4a", "#c8922f", "#7a3b8a", "#1b8a9c", "#a8452f", "#3f5d8a"];
  ...
  if (layout === "circle") {
    return (
      <div className="relative mx-auto mt-8 aspect-square w-full max-w-sm sm:max-w-md">
        <div className="absolute inset-[10%] rounded-full border-2 border-dashed border-navy/15" aria-hidden="true" />
        ...
        <div className="absolute inset-[19%] flex items-center justify-center rounded-full bg-card p-4 text-center shadow-md">
          {active && (
            <div>
              ...
              <p className="text-xs font-medium text-orange">{active.role}</p>
              {active.facts.length > 0 && (
                <div className="mt-1.5 flex flex-wrap justify-center gap-1">
                  {active.facts.map((f) => (
                    <span key={f.label} className="rounded-full bg-navy/5 px-2 py-0.5 text-[10px] font-semibold text-navy">
                      {f.label}: {f.value}
                    </span>
                  ))}
                </div>
              ...
              <Reveal key={p.id} delay={i * 80} className="flex-none md:flex-auto">
                <button
                  ...
                  className={`flex w-full items-center gap-3 rounded-full border px-3 py-2 text-left transition-colors ${
                    isActive ? "border-navy bg-navy/5" : "border-border bg-card hover:bg-background"
                  }`}
                >
                  <span
      ...
      <div className="mt-10 flex flex-col items-center gap-8 md:flex-row md:items-center">
        <div className="relative mx-auto aspect-square w-full max-w-[18rem] flex-none sm:max-w-xs">
          ...
          <div className="absolute inset-0 flex items-center justify-center" aria-hidden="true">
            <span className="text-5xl text-navy/10">♛</span>
          </div>
          {profiles.map((p, i) => {
            const angle = (i / profiles.length) * 2 * Math.PI - Math.PI / 2;
            const x = 50 + 46 * Math.cos(angle);
    ...
    <div className={`rounded-xl border border-border bg-card p-6 shadow-sm ${className}`}>
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm font-medium text-orange">{profile.role}</p>
        </div>
      </div>
      ...
      <div className="mt-6 border-t border-border pt-4">
        <Link href="/classes" className="text-sm font-semibold text-navy hover:underline">
